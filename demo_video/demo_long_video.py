...
# def process_folder(input_folder, output_folder, demo, windows_size):
#     os.makedirs(output_folder, exist_ok=True)

#     frames_path = glob.glob(os.path.expanduser(os.path.join(input_folder, '*.???')))
#     frames_path.sort()
#     if windows_size == -1:
#         windows_size = len(frames_path)
...
#             # 保存结果
#             for path, _vis_output in zip(_frames_path, visualized_output):
#                 out_filename = os.path.join(output_folder, os.path.basename(path))
#                 _vis_output.save(out_filename)
#             for id in predictions['pred_ids']:
#                 instances.add(id)
#             del visualized_output, vid_frames, _frames_path, predictions
...
#         input_folder = os.path.join(video_root, folder_name)
#         output_folder = os.path.join(output_root, folder_name)
#         process_folder(input_folder, output_folder, demo, windows_size)
