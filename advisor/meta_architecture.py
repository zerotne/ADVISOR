...
from.video_advisor_modules import ProgressiveTracker, RefinementCompensator, SpatialInteractionModule
...
@META_ARCH_REGISTRY.register()
class ADVISOR_offline(ADVISOR_online):
    ...
    def from_config(cls, cfg):
        ...
        compensator = RefinementCompensator(
            hidden_channel=cfg.MODEL.MASK_FORMER.HIDDEN_DIM,
            feedforward_channel=cfg.MODEL.MASK_FORMER.DIM_FEEDFORWARD,
            num_head=cfg.MODEL.MASK_FORMER.NHEADS,
            decoder_layer_num=cfg.MODEL.COMPENSATOR.DECODER_LAYERS,
            mask_dim=cfg.MODEL.MASK_FORMER.HIDDEN_DIM,
            class_num=cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,
            # windows=cfg.MODEL.MASK_FORMER.TEST.WINDOW_SIZE
        )

        interaction = SpatialInteractionModule(
            hidden_channel=cfg.MODEL.MASK_FORMER.HIDDEN_DIM,
            feedforward_channel=cfg.MODEL.MASK_FORMER.DIM_FEEDFORWARD,
            num_head=cfg.MODEL.MASK_FORMER.NHEADS,
            decoder_layer_num=cfg.MODEL.INTERACTION.DECODER_LAYERS,
            mask_dim=cfg.MODEL.MASK_FORMER.HIDDEN_DIM,
            class_num=cfg.MODEL.SEM_SEG_HEAD.NUM_CLASSES,
            # windows=cfg.MODEL.MASK_FORMER.TEST.WINDOW_SIZE
        )
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            with torch.no_grad():
                ...
                torch.cuda.empty_cache()
            # do temporal refine
            outputs = self.compensator(instance_embeds, frame_embds_, mask_features)
            outputs = self.interaction(instance_embeds, frame_embds_, mask_features)
    ...
    def run_window_inference(self, images_tensor, window_size=30):
        ...
        outputs1 = self.compensator(overall_instance_embds, overall_frame_embds_, overall_mask_features)
        outputs2 = self.interaction(overall_instance_embds, overall_frame_embds_, overall_mask_features)
